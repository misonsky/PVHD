...
class ContextEncoder(keras.Model):
    def __init__(self,output_size,config,bidirectional=False):
        ...
        self.encoder = BASICEncoder(rnn_type=config.rnn_type,
                                    ...
                                    num_layers=config.encoder_layers,
                                    bidirectional=bidirectional)
        self.attn = BahdanauAttention(config.d_model)
    def call(self, x, mask,hidden=None):
        ...
        """
        output, state = self.encoder(x,mask=mask,initial_state = hidden)
        nums_turns = tf.reduce_sum(mask,axis=1)
        last_hidden = []
        for out,_index in zip(tf.unstack(output, axis=0),tf.unstack(nums_turns,axis=0)):
            t = tf.gather(out,[tf.cast(_index-1,dtype=tf.int32)],axis=0)
            last_hidden.append(tf.reshape(t, shape=[t.shape[-1]]))
        last_hidden = tf.stack(last_hidden, axis=0) #batch *hidden
        static_attn = self.attn(last_hidden,output) #batch * hidden
        return static_attn,output, state 
...
class DSHRED(keras.Model):
    def __init__(self,vocab_size,embedding_dim,matrix,config,SOS=0,EOS=0,PAD=0):
        ...
        self.embedding = tf.keras.layers.Embedding(vocab_size, 
                                                   ...
                                                   trainable=True)
        self.encoder1 = UttEncoder(embedFunction=self.embedding,
                                output_size = config.d_model,
                                config=config,
                                bidirectional=config.bidirectional)
        self.encoder2 = ContextEncoder(output_size = config.d_model,
                                config=config,
                                bidirectional=config.bidirectional)
    ...
    def call(self,features,training=True):
        ...
        static_attn,enc_output, enc_hidden = self.encoder2(utterances,mask=context_mask)
    ...
    def BeamDecoder(self,features,training=False):
        ...
        states = {"dec_hidden":dec_hidden,"enc_output":enc_output,"static_attn":static_attn}
        def symbols_to_logits_fn(tgtids, i, states):
            ...
            decoderOut, dec_hidden = self.decoder(x = dec_input, 
                        ...
                        static_attn = states["static_attn"])
            logits = self.outputLayer(decoderOut)#batch * vocab

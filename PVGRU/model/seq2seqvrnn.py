...
class Encoder(keras.Model):
    def __init__(self,embedFunction,output_size,config,bidirectional=False):
        ...
        self.embedding = embedFunction
        self.encoder = BASICEncoder(rnn_type="gru",
                                    output_size=output_size,
                                    num_layers=config.num_layers,
                                    bidirectional=bidirectional)
    def call(self, x, mask,hidden=None,useEmbedding=True):
...
class Decoder(keras.Model):
    def __init__(self,embedFunction,output_size,config,bidirectional=False):
        ...
        self.embedding = embedFunction
        self.decoder = BASICEncoder(rnn_type="gru",
                                    output_size=output_size,
                                    num_layers=config.num_layers,
                                    bidirectional=bidirectional)
    def call(self, x, hidden):
...
class VarHRED(keras.Model):
    ...
    def call(self,features,training=True):
        ...
        for utt in src:
            ...
            utterances.append(tf.reshape(states,shape=[states.shape[0],states.shape[1] * states.shape[-1]]))
    ...
    def BeamDecoder(self,features,training=False):
        ...
        vstates,vlatents = self.separate_latent_state(voutputs[1:], bidirectional=self.config.bidirectional)
...
class VarSeq2Seq(keras.Model):
    def __init__(self,vocab_size,embedding_dim,matrix,config,SOS=0,EOS=0,PAD=0):
        super(VarSeq2Seq, self).__init__()
        self.embedding = tf.keras.layers.Embedding(vocab_size, 
                                                   embedding_dim,
                                                   embeddings_initializer=keras.initializers.constant(matrix),
                                                   trainable=True)

...
class VAD(keras.Model):
    ...
    def call(self,features,training=True):
        ...
        _, enc_hidden = self.encoder2(utterances,mask=context_mask,useEmbedding=False)
        ...
        for t in range(1, tgt.shape[1]):
            decoderOut,dec_hidden,kld_loss = self.decoder(x = dec_input, hidden = dec_hidden)
            kl_loss += kld_loss
    ...
    def BeamDecoder(self,features,training=False):
        ...
        def symbols_to_logits_fn(tgtids, i, states):
            ...
            decoderOut, dec_hidden,_ = self.decoder(x = dec_input,hidden = states["dec_hidden"])
            logits = self.outputLayer(decoderOut)#batch * vocab
            states["dec_hidden"] = dec_hidden

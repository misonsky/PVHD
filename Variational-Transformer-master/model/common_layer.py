...
from utils.EvaluationUtils import calculationEmbedding,cal_Distinct,compute_bleu_rouge_single_prediction
#from beam_omt import Translator
# import matplotlib.pyplot as plt

def EvaluateDialogue(config,golds:List[str],predictions:List[str]):
    golds = [word_tokenize(gold) for gold in golds]
...
def print_custum(dial,ref,hyp_g):
...
def evaluate_tra(model, data,  ty='train', max_dec_step=50):
    ...
    for j, batch in enumerate(data):
        ...
        for i, greedy_sent in enumerate(sent_g):
            ...
            print_custum(dial=[" ".join(s) for s in batch['input_txt'][i]] if config.datasets=="empathetic" else " ".join(batch['input_txt'][i]),
                        ref=rf,
                        hyp_g=greedy_sent)  
        if i>10:
            break 
...
def evaluate(model, data,  ty='valid', max_dec_step=50):
    ...
    bow = np.mean(bow)
    elbo = np.mean(elbo)
    metricsResults = EvaluateDialogue(config=config,golds=ref,predictions=hyp_g)
    metricsResults["loss"] = loss
    metricsResults["ppl"] = ppl
    metricsResults["kld"] = kld
    metricsResults["bow"] = bow
    metricsResults["elbo"] = elbo
